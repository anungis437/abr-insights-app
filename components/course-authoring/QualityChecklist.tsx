...
export default function QualityChecklist({
  ...
  const getCategoryProgress = (category: ChecklistCategory): number => {
    ...
    const completed = category.items.filter((item) => checklist[item.id] === true).length
    ...
    <div className="space-y-4">
      ...
      <div className={`rounded-lg border p-4 ${getCompletionColor(completionPercentage)}`}>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm opacity-90">
                {completionPercentage}% complete ({Math.round((completionPercentage / 100) * 19)} of
                19 items)
              </p>
            </div>
          </div>
        ...
        {showSubmitThreshold && !meetsThreshold && (
          <div className="mt-3 flex items-start gap-2 text-sm">
            ...
            <p>
              <strong>Note:</strong> Minimum {SUBMIT_THRESHOLD}% completion required to submit for
              review. Complete at least{' '}
              {Math.ceil(((SUBMIT_THRESHOLD - completionPercentage) / 100) * 19)} more items.
            </p>
          </div>
        )}
            ...
            >
              ...
              >
                <div className="flex items-center gap-3 text-left">
                  <div className={`h-2 w-2 rounded-full bg-${category.color}-500`}></div>
                  <div>
                ...
                <div className="flex items-center gap-3">
                  <div className="text-right">
                    ...
                    <div className="text-xs text-gray-500">
                      {category.items.filter((item) => checklist[item.id] === true).length}/
      ...
      {checklist.checked_at && (
        <div className="border-t border-gray-200 pt-2 text-center text-sm text-gray-500">
          Last updated:{' '}
          {new Date(checklist.checked_at).toLocaleString('en-CA', {
            dateStyle: 'medium',
            timeStyle: 'short',
          })}
        </div>
