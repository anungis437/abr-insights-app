...
async function analyzeAIUsage() {
  ...
  const { data, error } = await supabase
    .from('cases')
    .select(
      'case_title, ai_classification, rule_based_classification, combined_confidence, needs_review, created_at'
    )
  ...
  // Category breakdown
  const antiBlackRacism = data.filter(
    (c) =>
      c.rule_based_classification?.category === 'anti_black_racism' ||
      c.ai_classification?.category === 'anti_black_racism'
  ).length

  const aiAntiBlackRacism = data.filter(
    (c) => c.ai_classification?.category === 'anti_black_racism'
  ).length

  const ruleAntiBlackRacism = data.filter(
    (c) => c.rule_based_classification?.category === 'anti_black_racism'
  ).length
  ...
  console.log(
    `Flagged for Review:       ${needsReview} (${((needsReview / total) * 100).toFixed(1)}%)`
  )
  console.log('')
  console.log('CLASSIFICATION BREAKDOWN:')
  console.log(
    `Anti-Black Racism (Any):  ${antiBlackRacism} (${((antiBlackRacism / total) * 100).toFixed(1)}%)`
  )
  ...
  recentAI.forEach((c, idx) => {
    console.log(`\n${idx + 1}. ${c.case_title}`)
    console.log(
      `   Rule: ${c.rule_based_classification?.category} (${c.rule_based_classification?.confidence})`
    )
