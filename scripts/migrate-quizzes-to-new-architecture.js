...
async function migrateAllQuizzes() {
  ...
  try {
            ...
            if (options.length > 0) {
              for (let oIndex = 0; oIndex < options.length; oIndex++) {
                ...
                const { error: optionError } = await supabase.from('question_options').insert({
                  ...
                  order_index: oIndex,
                  explanation: oIndex === correctAnswer && q.explanation ? q.explanation : null,
                })
    ...
    const { data: quizzesCheck } = await supabase.from('quizzes').select('id')

    const { data: questionsCheck } = await supabase.from('quiz_questions').select('id')
