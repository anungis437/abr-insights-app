...
function extractColumnDefinitions(createTableSql) {
  ...
  while ((match = columnPattern.exec(createTableSql)) !== null) {
    ...
    if (
      !['PRIMARY', 'FOREIGN', 'UNIQUE', 'CHECK', 'CONSTRAINT', 'REFERENCES'].includes(
        name.toUpperCase()
      )
    ) {
...
function parseAlterTable(sql, file) {
  ...
  const addColumnMatch = sql.match(
    /ADD\s+COLUMN\s+(?:IF\s+NOT\s+EXISTS\s+)?["']?(\w+)["']?\s+([\w\[\]]+(?:\([^)]*\))?)/i
  )
  if (addColumnMatch) {
    ...
    if (schemaState.columns.has(key)) {
      log(
        'warnings',
        `Column "${colName}" already exists in "${tableName}" - ADD COLUMN may fail`,
        file
      )
    } else {
...
function parseRLSPolicy(sql, file) {
  const policyMatch = sql.match(
    /CREATE\s+POLICY\s+["']?([^"'\s]+)["']?\s+ON\s+(?:public\.)?["']?(\w+)["']?/i
  )
...
function parseDropPolicy(sql, file) {
  const dropMatch = sql.match(
    /DROP\s+POLICY\s+(?:IF\s+EXISTS\s+)?["']?([^"'\s]+)["']?\s+ON\s+(?:public\.)?["']?(\w+)["']?/i
  )
  if (!dropMatch) return
  ...
  if (tableRefs) {
    tableRefs.forEach((ref) => {
      ...
      if (!['dual', 'pg_', 'information_schema'].some((skip) => tableName.startsWith(skip))) {
        if (
          !schemaState.tables.has(tableName) &&
  ...
  return client
    ...
    .then((result) => {
      ...
      if (existingTables.length > 0) {
        log(
          'warnings',
          `Database already has ${existingTables.length} tables - migrations may conflict`
        )
