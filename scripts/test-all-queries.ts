...
async function testQueries() {
  ...
  const { data: wh2, error: whError2 } = await supabase
    ...
    .eq('user_id', userId)
    .limit(1);
  console.log(whError2 ? `Error: ${whError2.message}` : `Columns: ${wh2?.[0] ? Object.keys(wh2[0]).join(', ') : 'no data'}`);
  ...
  const { data: lp2, error: lpError2 } = await supabase
    ...
    .eq('user_id', userId)
    .limit(1);
  console.log(lpError2 ? `Error: ${lpError2.message}` : `Columns: ${lp2?.[0] ? Object.keys(lp2[0]).join(', ') : 'no data'}`);

  // Test 7: Check actual columns in user_points
  console.log('\n7. Testing user_points all columns...');
  const { data: up2, error: upError2 } = await supabase
    .from('user_points')
    .select('*')
    .limit(1);
  console.log(upError2 ? `Error: ${upError2.message}` : `Columns: ${up2?.[0] ? Object.keys(up2[0]).join(', ') : 'no data'}`);
}
