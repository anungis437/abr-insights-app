...
async function checkSchemas() {
  ...
  const { data: wh, error: whError } = await supabase.from('watch_history').select('*').limit(0)
  ...
  const { data: lp, error: lpError } = await supabase.from('lesson_progress').select('*').limit(0)
  ...
  const { data: up, error: upError } = await supabase.from('user_points').select('*').limit(0)
  ...
  const { data: testQuery, error: testError } = await supabase
    .from('watch_history')
    .select(
      'id,session_start,duration_seconds,completed,lesson:lessons(id,title,course_module:course_modules(course:courses(title)))'
    )
