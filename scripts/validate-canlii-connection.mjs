...
async function validateApiKey() {
  ...
  log(
    `✅ API Key found: ${API_KEY.substring(0, 8)}...${API_KEY.substring(API_KEY.length - 4)}`,
    colors.green
  )
  return true
}
...
async function testDatabaseListing() {
  ...
  try {
    ...
    if (response.status === 200 && response.data?.caseDatabases) {
      ...
      const tribunals = databases.filter(
        (db) =>
          db.name.toLowerCase().includes('tribunal') || db.name.toLowerCase().includes('board')
      )
