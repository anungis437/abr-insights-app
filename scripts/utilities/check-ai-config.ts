...
console.log(
  `  AZURE_OPENAI_ENDPOINT:        ${hasEndpoint ? '✅ Set' : '❌ Missing'} ${hasEndpoint ? `(${ENV.AZURE_OPENAI_ENDPOINT})` : ''}`
)
console.log(
  `  AZURE_OPENAI_API_KEY:         ${hasApiKey ? '✅ Set' : '❌ Missing'} ${hasApiKey ? '(hidden)' : ''}`
)
console.log(
  `  AZURE_OPENAI_DEPLOYMENT_NAME: ${hasDeployment ? '✅ Set' : '❌ Missing'} ${hasDeployment ? `(${ENV.AZURE_OPENAI_DEPLOYMENT})` : ''}`
)
console.log(
  `  AZURE_OPENAI_API_VERSION:     ${hasApiVersion ? '✅ Set' : '❌ Missing'} ${hasApiVersion ? `(${ENV.AZURE_OPENAI_API_VERSION})` : ''}`
)
