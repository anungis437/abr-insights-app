...
async function migrateAllQuizzes() {
  ...
  try {
      ...
      for (const quiz of quizzes) {
        ...
        for (let qIndex = 0; qIndex < contentData.questions.length; qIndex++) {
          ...
          try {
            ...
            const correctAnswerIndex = typeof q.correct_answer === 'number' 
              ? q.correct_answer 
    ...
    if (questionsCheck && questionsCheck.length === stats.questionsCreated) {
      ...
      console.log(`   Expected ${stats.questionsCreated} questions, found ${questionsCheck ? questionsCheck.length : 0}`);
    }
