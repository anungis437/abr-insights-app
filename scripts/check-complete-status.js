...
async function checkCompleteStatus() {
  ...
  const { count: moduleCount } = await supabase.from('course_modules').select('id', { count: 'exact' })
  ...
  // Check each phase
  const phases = [
    { name: 'Phase 1 (Foundational)', slugs: [
  ...
  for (const phase of phases) {
    ...
    for (const slug of phase.slugs) {
      ...
      const { count: mCount } = await supabase.from('course_modules').select('id', { count: 'exact' }).eq('course_id', course.id)
      const { count: lCount } = await supabase.from('lessons').select('id', { count: 'exact' }).eq('course_id', course.id)
