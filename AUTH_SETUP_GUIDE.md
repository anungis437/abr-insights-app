...
3. Copy the correct keys:
   - **Project URL**: Should be `https://zdcmugkafbczvxcyofiz.supabase.co`
   - **anon public key**: JWT token starting with `eyJ...`
   - **service_role secret**: JWT token starting with `eyJ...`
...
```bash
curl -X POST https://zdcmugkafbczvxcyofiz.supabase.co/auth/v1/token?grant_type=password \
