...
test.describe('AI Quota Enforcement', () => {
  ...
  test('should allow AI request when under quota', async ({ page }) => {
    ...
    const hasQuotaError = await page
      .getByText(/quota exceeded|limit reached/i)
      .isVisible()
      .catch(() => false)
  ...
  test('should show quota exceeded message in UI', async ({ page }) => {
    ...
    if (!isDisabled) {
      ...
      await expect(page.getByText(/quota exceeded|limit reached|100 messages/i)).toBeVisible({
        timeout: 5000,
      })
    } else {
