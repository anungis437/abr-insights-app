...
describe.skipIf(skipTests)('Tenant Isolation Tests', () => {
  beforeAll(async () => {
    ...
    if (orgError || !orgs || orgs.length === 0) {
      throw new Error(
        `Failed to create test organizations: ${orgError?.message || 'No orgs returned'}`
      )
    }
    ...
    const { error: profile1Error } = await adminClient.from('profiles').insert({
      id: user1.id,
    ...
    const { error: profile2Error } = await adminClient.from('profiles').insert({
      id: user2.id,
    ...
    if (coursesError || !courses || courses.length === 0) {
      throw new Error(
        `Failed to create test courses: ${coursesError?.message || 'No courses returned'}`
      )
    }
  ...
  describe('Cross-Tenant Profile Access', () => {
    it('should only see own profile', async () => {
      const { data: profiles } = await user1Client.from('profiles').select('id')
    ...
    it('should not access other org profiles', async () => {
      const { data } = await user1Client.from('profiles').select('id').eq('id', user2Id)
  ...
  describe('Cross-Tenant Organization Access', () => {
    it('should only see own organization', async () => {
      const { data: orgs, error } = await user1Client.from('organizations').select('id')
    ...
    it('should not access other organizations', async () => {
      const { data, error } = await user1Client.from('organizations').select('id').eq('id', org2Id)
  ...
  describe('Enrollment Isolation', () => {
    ...
    it('should only see own enrollments', async () => {
      const { data: enrollments } = await user1Client.from('enrollments').select('id')
    ...
    it('should not access other user enrollments', async () => {
      const { data } = await user1Client.from('enrollments').select('id').eq('id', enrollment2)
  ...
  describe('Gamification Data Isolation', () => {
    ...
    it('should only see own points', async () => {
      const { data: points } = await user1Client.from('user_course_points').select('id')
  ...
  describe('Audit Log Isolation', () => {
    ...
    it('should only see own audit logs', async () => {
      ...
      const { data: logs } = await user1Client.from('audit_logs').select('id')
    ...
    it('should not access other org audit logs', async () => {
      ...
      const { data } = await user1Client.from('audit_logs').select('id').eq('id', logOrg2)
  ...
  describe('Permission Boundary Tests', () => {
    it('should enforce permission-based SELECT policies', async () => {
      ...
      const { data } = await user1Client.from('tribunal_cases').select('id').limit(10)
    ...
    it('should prevent unauthorized INSERT', async () => {
      ...
      const { error } = await user1Client.from('tribunal_cases').insert({
        title: 'Test Case',
  ...
  describe('RLS Policy Verification', () => {
    it('should have RLS enabled on critical tables', async () => {
      ...
      for (const table of criticalTables) {
        const { data } = await (adminClient as any).rpc('exec_sql', {
    ...
    it('should have permission-based policies', async () => {
      ...
      const { error } = await user1Client.from('courses').insert({
