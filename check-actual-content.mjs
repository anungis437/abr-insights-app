...
async function checkContent() {
  ...
  for (const course of courses || []) {
    ...
    const withArticleBody = lessons?.filter(l => l.article_body && l.article_body.length > 200).length || 0
    const withContentData = lessons?.filter(l => l.content_data && Object.keys(l.content_data).length > 0).length || 0
  ...
  complete.forEach((c, i) => {
    ...
    console.log(`   Content: ${c.withArticleBody} with article_body, ${c.withContentData} with content_data\n`)
  })
  
  if (withSomeContent.length > 0) {
    console.log('\n📝 COURSES WITH PARTIAL CONTENT:')
    console.log('==================================')
    withSomeContent.filter(c => !c.complete).forEach((c, i) => {
      console.log(`${i + 1}. ${c.title}`)
      console.log(`   Slug: ${c.slug}`)
      console.log(`   Lessons: ${c.lessons} total`)
      console.log(`   Content: ${c.withArticleBody} with article_body, ${c.withContentData} with content_data\n`)
