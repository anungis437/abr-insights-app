...
export class GamificationService {
   ...
   * Update user streak
   */
  async updateUserStreak(userId: string, streakType: string, activityDate?: string): Promise<void> {
    ...
    try {
      ...
      if (error) {
        ...
        if (error.code === 'PGRST301' || error.code === '42883') {
          console.warn(
            '[Gamification] get_user_points_summary function not found, returning default'
          )
          return {
            current_balance: 0,
            total_earned: 0,
  ...
  async getRewards(featured = false): Promise<Reward[]> {
    let query = this.supabase.from('rewards_catalog').select('*').eq('is_active', true)
  ...
  async getLeaderboards(type?: string): Promise<Leaderboard[]> {
    let query = this.supabase.from('leaderboards').select('*').eq('is_active', true)
