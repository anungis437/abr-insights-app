...
export async function reorderCourseModules(
  ...
  const promises = moduleOrders.map(({ id, order_index }) =>
    supabase.from('course_modules').update({ order_index }).eq('id', id)
  )
 ...
 */
export async function calculateCourseCompletion(userId: string, courseId: string): Promise<number> {
  const supabase = createClient()
...
export async function replyToDiscussion(
  ...
  const { data, error } = await supabase.from('course_discussions').insert(reply).select().single()
