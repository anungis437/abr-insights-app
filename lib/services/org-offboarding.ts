...
class OrgOffboardingService {
  ...
  async initiateOffboarding(request: OffboardingRequest): Promise<OffboardingStatus> {
    ...
    try {
      ...
      const { data: existing } = await this.supabase
        ...
        .in('status', ['requested', 'exporting_data', 'export_ready', 'cancelling_stripe', 'revoking_access'])
        .single()
      ...
      const { data: offboardingRequest, error: createError } = await this.supabase
        .from('org_offboarding_requests')
        .insert({
          organization_id: request.organizationId,
          requested_by: request.requestedBy,
  ...
  private async processOffboarding(
    ...
    try {
      ...
      await this.logAction(requestId, 'offboarding_failed', {
        error: errorMessage,
      }, false, errorMessage)
  ...
  private async cancelStripeSubscription(
    ...
    await this.logAction(requestId, 'stripe_cancellation_started', { organization_id: organizationId })
    ...
    try {
      ...
      if (org?.stripe_subscription_id) {
        ...
        await this.logAction(requestId, 'stripe_no_subscription', { organization_id: organizationId })
      }
  ...
  private async revokeOrganizationAccess(
    ...
    await this.logAction(requestId, 'access_revocation_started', { organization_id: organizationId })
    ...
    try {
      ...
      const { data: userOrgs, error: usersError } = await this.supabase
  ...
  async getStatusByOrganization(organizationId: string): Promise<OffboardingStatus | null> {
    const { data, error } = await this.supabase
      ...
      .select('*')
      .eq('organization_id', organizationId)
      .in('status', ['requested', 'exporting_data', 'export_ready', 'cancelling_stripe', 'revoking_access', 'deletion_scheduled'])
  ...
  private async updateStatus(
    ...
    await this.supabase.rpc('update_offboarding_status', {
      request_id: requestId,
      new_status: status,
      error_msg: error,
    })
  ...
  private async logAction(
    ...
    await this.supabase.rpc('log_offboarding_action', {
      request_id: requestId,
      action_name: action,
