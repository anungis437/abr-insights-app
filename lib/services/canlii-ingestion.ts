...
class CanLIIIngestionService {
  ...
  async startIngestion(options: IngestionRunOptions = {}): Promise<IngestionRunResult> {
    ...
    try {
      ...
      // Update run status
      await this.supabase.rpc('update_ingestion_run_status', {
        run_id: run.id,
   ...
   */
  private async fetchCaseMetadata(
    runId: string,
    caseId: string
    ...
    try {
      ...
      if (!response.ok) {
        ...
        await this.supabase.rpc('record_canlii_request', {
          run_id: runId,
  ...
  async getStats(): Promise<{
    ...
    // Get daily quota
    const { data: dailyQuota } = await this.supabase.rpc('get_canlii_daily_quota')
