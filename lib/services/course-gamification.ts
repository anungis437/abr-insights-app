...
export interface CourseGroupChallenge {
  ...
  title: string;
  description?: string;
  challenge_type: 'completion_race' | 'points_competition' | 'streak_challenge' | 'accuracy_challenge';
  target_value: number;
  start_date: string;
  end_date: string;
...
export class CourseGamificationService {
   ...
   */
  async calculateLeaderboard(leaderboardId: string): Promise<{ success: boolean; entries: number }> {
   ...
   */
  async getGamificationSummary(userId: string, courseId?: string): Promise<{
    ...
    if (courseId) {
      ...
      pointsData = { current_balance: totalPoints, level: Math.max(...allPoints.map(p => p.level), 1) };
    }
