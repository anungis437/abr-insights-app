...
import type { RuleBasedClassification, DiscriminationGround, DecisionContent } from '../types'
...
export class RuleBasedClassifier {
  ...
  classify(content: DecisionContent): RuleBasedClassification {
    ...
    const isRaceRelated =
      keywordMatches.raceKeywords.length > 0 && keywordMatches.discriminationKeywords.length > 0

    const isAntiBlackLikely = keywordMatches.blackKeywords.length > 0 && isRaceRelated
  ...
  private calculateConfidence(
    ...
    const hasBlackAndRace =
      keywordMatches.blackKeywords.length > 0 && keywordMatches.raceKeywords.length > 0
