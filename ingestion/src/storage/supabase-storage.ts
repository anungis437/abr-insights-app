...
export class SupabaseStorageService {
  ...
  async updateJobMetrics(
    ...
    const { error } = await this.client.from('ingestion_jobs').update(metrics).eq('id', jobId)
  ...
  async completeJob(
    ...
    const { error } = await this.client.from('ingestion_jobs').update(updates).eq('id', jobId)
  ...
  async storeCase(
    ...
    const caseData: Partial<TribunalCaseRaw> = {
      ...
      // Quality Flags
      extraction_quality:
        content.fullText.length > 5000 ? 'high' : content.fullText.length > 2000 ? 'medium' : 'low',
      extraction_errors: [],
  ...
  async logError(
    ...
    const { error: insertError } = await this.client.from('ingestion_errors').insert(errorData)
