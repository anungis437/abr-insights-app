...
const INTERSECTIONAL_GROUNDS = [
  {
    ...
    description: 'Black person with disability facing compounded discrimination',
  },
  {
    primary: 'race',
    secondary: 'sex',
    description: 'Black woman experiencing both racial and gender discrimination',
  },
  {
    primary: 'race',
    secondary: 'age',
    description: 'older Black employee facing age and racial discrimination',
  },
  {
    primary: 'race',
    secondary: 'creed',
    description: 'Black Muslim facing religious and racial discrimination',
  },
  {
    primary: 'race',
    secondary: 'ancestry',
    description: 'African immigrant facing xenophobic and anti-Black racism',
  },
]
...
const OUTCOME_TEMPLATES: OutcomeTemplate[] = [
  ...
  {
    type: 'decision',
    description: 'Application allowed - significant damages awarded',
    hasRemedies: true,
  },
  {
    type: 'decision',
    description: 'Application allowed in part - limited remedies ordered',
    hasRemedies: true,
  },
  {
    type: 'decision',
    description: 'Application dismissed - insufficient evidence',
    hasRemedies: false,
  },
  {
    type: 'decision',
    description: 'Application dismissed - legitimate non-discriminatory reasons',
    hasRemedies: false,
  },
  ...
  {
    type: 'settlement',
    description: 'Matter resolved through settlement agreement',
    hasRemedies: true,
  },
  {
    type: 'settlement',
    description: 'Confidential settlement reached by parties',
    hasRemedies: true,
  },
  ...
  {
    type: 'mediation',
    description: 'Mediation successful - parties reached agreement',
    hasRemedies: true,
  },
  ...
  {
    type: 'withdrawal',
    description: 'Application withdrawn without prejudice',
    hasRemedies: false,
  },
...
function generateCaseNumber(sourceSystem: SourceSystem, year: number): string {
  const tribunalCode = sourceSystem.includes('hrto')
    ? 'HRTO'
    : sourceSystem.includes('chrt')
      ? 'CHRT'
      : sourceSystem.includes('bchrt')
        ? 'BCHRT'
        : sourceSystem.includes('abhr')
          ? 'AHRC'
          : sourceSystem.includes('skhr')
            ? 'SKHR'
            : sourceSystem.includes('mbhr')
              ? 'MBHR'
              : sourceSystem.includes('qctdp')
                ? 'QCTDP'
                : sourceSystem.includes('nshr')
                  ? 'NSHR'
                  : sourceSystem.includes('nbhr')
                    ? 'NBHR'
                    : 'DEMO'
...
function generateDecisionText(
  ...
  if (language === 'fr') {
    return generateFrenchDecisionText(
      applicant,
      respondent,
      issue,
      ground,
...
export function generateDemoContent(
  ...
  const intersectionalGrounds =
    options.intersectional && shouldHaveAntiBlackRacism && Math.random() > 0.3
  ...
  const outcomeType =
    options.outcomeType ||
    (shouldHaveAntiBlackRacism && Math.random() > 0.7
  ...
  const partialSuccess =
    options.partialSuccess !== undefined
  ...
  const outcomeTemplate =
    OUTCOME_TEMPLATES.find(
      (t) => t.type === outcomeType && t.hasRemedies === shouldHaveAntiBlackRacism
    ) || OUTCOME_TEMPLATES[0]
  ...
  const tribunal = sourceSystem.includes('hrto')
  ...
  const province = sourceSystem.includes('hrto')
    ? 'ON'
    : sourceSystem.includes('chrt')
      ? 'CA'
      : sourceSystem.includes('bchrt')
  ...
  return links.map((link, index) => {
    ...
    if (includeSettlements && shouldHaveAntiBlackRacism) {
      ...
      if (Math.random() > 0.8) {
        ...
        contentOptions.outcomeType =
          settlementTypes[Math.floor(Math.random() * settlementTypes.length)]
      }
    }
    ...
    const content = generateDemoContent(
      link,
      sourceSystem,
      shouldHaveAntiBlackRacism,
      contentOptions
