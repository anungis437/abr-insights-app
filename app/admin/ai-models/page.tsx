...
import type { ClassificationFeedback, TrainingJob, AutomatedTrainingConfig } from '@/lib/ai/training-service'
...
export default function AIModelManagementPage() {
  ...
  const [processingLog, setProcessingLog] = useState<Array<{ timestamp: string; message: string; type: string }>>([])
  ...
  const submitFineTuningJob = async () => {
    ...
    try {
      ...
      const trainingExamples = feedbackData.map(feedback => ({
        messages: [
          { role: 'system', content: 'You are a Canadian human rights analyst specializing in anti-Black racism cases.' },
          { role: 'user', content: `Analyze this case: ${feedback.case_title}\n\n${feedback.case_text_excerpt}` },
      ...
      const { data: trainingUrl } = supabase.storage.from('ai-training').getPublicUrl(trainingUpload.path)
      const { data: validationUrl } = supabase.storage.from('ai-training').getPublicUrl(validationUpload.path)
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                  ...
                  <div className="p-4 bg-yellow-50 border-2 border-yellow-300 rounded-lg flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-yellow-800 mt-1">
                        You need at least 10 high-quality feedback records to start training. Currently have: {readyCount}
                      </p>
                    </div>
                  </div>
                )}
                ...
                <div className="grid md:grid-cols-2 gap-4">
                  <div>
                    <label className="text-sm font-medium text-gray-700 mb-2 block">Job Name</label>
                    <Input
                  ...
                  <div>
                    <label htmlFor="base-model-select" className="text-sm font-medium text-gray-700 mb-2 block">Base Model</label>
                    <select
                ...
                <div className="border-t pt-4">
                  ...
                  <div className="grid md:grid-cols-3 gap-4">
                    <div>
                      <label className="text-xs font-medium text-gray-700 mb-1 block">Epochs</label>
                      <Input
                        type="number"
                        min="1"
                        max="10"
                    ...
                    <div>
                      <label className="text-xs font-medium text-gray-700 mb-1 block">Learning Rate Multiplier</label>
                      <Input
                        type="number"
                        min="0.1"
                        max="2.0"
          ...
          <TabsContent value="automation" className="space-y-6">
            ...
            {showAutomationConfig && (
              <Card>
                ...
                <CardContent className="space-y-4">
                  <div>
                    <label className="text-sm font-medium text-gray-700 mb-2 block">Configuration Name</label>
                    <Input
                      value={automationForm.config_name}
                      onChange={(e) => setAutomationForm({ ...automationForm, config_name: e.target.value })}
                    />
                  </div>

                  <div className="grid md:grid-cols-2 gap-4">
                    <div>
                      <label htmlFor="schedule-type-select" className="text-sm font-medium text-gray-700 mb-2 block">Schedule Type</label>
                      <select
                        id="schedule-type-select"
                        value={automationForm.schedule_type}
                        onChange={(e) => setAutomationForm({ ...automationForm, schedule_type: e.target.value as any })}
                        className="w-full px-3 py-2 border border-gray-300 rounded-lg"
                      >
                    ...
                    <div>
                      <label className="text-sm font-medium text-gray-700 mb-2 block">Run at Hour (24h)</label>
                      <Input
                        type="number"
                        min="0"
                        max="23"
                        value={automationForm.schedule_hour}
                        onChange={(e) => setAutomationForm({ ...automationForm, schedule_hour: Number(e.target.value) })}
                      />
                    </div>
                  </div>
                  ...
                  <div className="grid md:grid-cols-2 gap-4">
                    <div>
                      <label className="text-sm font-medium text-gray-700 mb-2 block">Feedback Threshold</label>
                      <Input
                        type="number"
                        min="10"
                        value={automationForm.feedback_threshold}
                        onChange={(e) => setAutomationForm({ ...automationForm, feedback_threshold: Number(e.target.value) })}
                      />
                    </div>

                    <div>
                      <label className="text-sm font-medium text-gray-700 mb-2 block">Min Quality Score</label>
                      <Input
                        type="number"
                        min="1"
                        max="5"
                        value={automationForm.min_quality_score}
                        onChange={(e) => setAutomationForm({ ...automationForm, min_quality_score: Number(e.target.value) })}
                      />
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    <input
                      ...
                      onChange={(e) => setAutomationForm({ ...automationForm, auto_deploy: e.target.checked })}
                      className="w-4 h-4"
                    />
                  ...
                  <div className="flex gap-3">
                    <Button
                      onClick={submitAutomationConfig}
                      className="bg-purple-600 text-white"
                    >
                      Save Configuration
                    </Button>
                  ...
                  <CardHeader>
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-sm text-gray-600">{job.version} • {job.base_model}</p>
                      </div>
                      <div className="flex items-center gap-2">
                        {job.status === 'pending' && <Badge className="bg-gray-100 text-gray-800">Pending</Badge>}
                        {job.status === 'running' && <Badge className="bg-blue-100 text-blue-800">Running</Badge>}
                        {job.status === 'succeeded' && <Badge className="bg-green-100 text-green-800">Succeeded</Badge>}
                        {job.status === 'failed' && <Badge className="bg-red-100 text-red-800">Failed</Badge>}
                        {job.is_deployed && <Badge className="bg-purple-100 text-purple-800">Deployed</Badge>}
                      </div>
                    </div>
                  </CardHeader>
                  <CardContent>
                    <div className="grid md:grid-cols-3 gap-4 text-sm">
                      <div><strong>Feedback Count:</strong> {job.feedback_count}</div>
                      <div><strong>Created:</strong> {new Date(job.created_at).toLocaleDateString()}</div>
                      {job.provider_job_id && <div><strong>Provider ID:</strong> {job.provider_job_id}</div>}
                    </div>
                    {job.notes && (
                      <p className="mt-4 text-sm text-gray-600">{job.notes}</p>
                    )}
                      ...
                      <div key={feedback.id} className="p-4 border rounded-lg hover:bg-gray-50">
                        <div className="flex items-start justify-between">
                          <div className="flex-1">
                            ...
                            <p className="text-sm text-gray-600 mt-1 line-clamp-2">{feedback.case_text_excerpt}</p>
                          </div>
                          <Badge className="ml-4">
                            Quality: {feedback.quality_score}/5
                          </Badge>
                        </div>
                      </div>
                    ))}
