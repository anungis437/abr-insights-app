...
async function verifyCompleteness() {
  ...
  for (const course of courses || []) {
    ...
    const withContentData = lessons?.filter(l => l.content_data && Object.keys(l.content_data).length > 0)
    ...
    // Completion check
    const isComplete = totalLessons >= 3 && (withArticleBody.length > 0 || withContentData.length > 0)
    console.log(`\n   Status: ${isComplete ? '✅ COMPLETE' : '❌ INCOMPLETE'}`)
    
    if (!isComplete) {
      console.log(`   ⚠️  Issue: ${totalLessons < 3 ? 'Not enough lessons' : 'No content in lessons'}`)
    }
  }
